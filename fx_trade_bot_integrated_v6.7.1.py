# fx_trade_bot_integrated_v6.7.1.py — AUTO-RANKING EDITION | v5-Style Transparency
# WEIGHTS: Strength 50% | RSI 15% | ADX 15% | XGBoost 15% | MC 5%
# STYLE: v5 — ALL pairs scanned | Full debug per component | Explicit PASS/FAIL reasons
...
parser = argparse.ArgumentParser(description="FX Trading Bot v6.7.1 | v5-Style Debug | AUTO-RANKING")
...
def main():
    ...
    logger.info(f"\n🤖 RUN v6.7.1 — {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')} | "
    ...
    if forex_market_closed():
        ...
        send_telegram_message("⏸️ FX BOT v6.7.1: Market closed")
        return
    ...
    # ─── STEP 8: Final Report ───
    if trade_lines:
        summary = f"🤖 v6.7.1 RUN COMPLETE — {signal_count} SIGNAL(S)\n\n" + "\n".join(trade_lines)
        send_telegram_message(summary)
    else:
        logger.info("📋 No signals passed thresholds this run")
        if args.test_trade:
            send_telegram_message("🤖 v6.7.1 TEST — No signals passed threshold (20.0)")

    logger.info("✅ v6.7.1 Run Complete")
