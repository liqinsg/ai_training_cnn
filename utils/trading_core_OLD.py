...
def forex_market_closed():
    """Check if Forex market is closed (UTC time)

    Closed:
      - Sunday all day
      - Saturday from 21:00 UTC onwards
    """
...
def open_oanda_order(signal, units):
    """Open order with SL/TP properly attached"""
    pair = signal["pair"].replace("_", "/")  # normalize format
    ...
    # ✅ ATTACH SL/TP AT TOP LEVEL — OANDA REQUIRES THIS
    if sl is not None:
        order["stopLossOnFill"] = {
            "price": str(round(float(sl), 5)),
            "timeInForce": "GTC"
        }
    if tp is not None:
        order["takeProfitOnFill"] = {
            "price": str(round(float(tp), 5)),
            "timeInForce": "GTC"
        }
    ...
    resp = api.request(OrderCreate(accountID=api.account_id, data=request))
...
def get_open_instruments():
    try:
        ...
        # Use the same account ID your code already loads
        acc_id = getattr(api, "account_id", OANDA_ACCOUNT_ID)
        req = OpenPositions(accountID=acc_id)
