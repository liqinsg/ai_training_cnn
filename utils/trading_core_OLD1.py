...
def execute_market_trade(signal, units_override=None):
    ...
    try:
        ...
        if (
            signal.action == "BUY" and (signal.stop_loss >= ask or signal.take_profit <= ask)
        ) or (
...
def open_oanda_order(signal, units):
    """Submit market order with properly formatted SL/TP at TOP LEVEL"""
    instr = signal["pair"]
    ...
    # ✅ Correct format + OANDA‑required top‑level keys
    if sl is not None:
        order["stopLossOnFill"] = {
            "price": format_price_for_instrument(sl, instr),
            "timeInForce": "GTC"
        }
    if tp is not None:
        order["takeProfitOnFill"] = {
            "price": format_price_for_instrument(tp, instr),
            "timeInForce": "GTC"
        }

    try:
        resp = oanda_client.request(orders.OrderCreate(
            accountID=OANDA_ACCOUNT_ID,
            data={"order": order}
        ))
        if "orderFillTransaction" in resp:
            return {"status": "OK", "id": resp["orderFillTransaction"]["id"]}
        return {"status": "ERROR", "message": "No fill in response"}
    except Exception as e:
        return {"status": "ERROR", "message": str(e)}

def forex_market_closed():
    """Check if Forex market is closed by UTC time — reliable, no API call"""
    now = datetime.utcnow()
    wd = now.weekday()  # 0=Mon, 5=Sat, 6=Sun
    return wd == 6 or (wd == 5 and now.hour >= 21)

def get_open_instruments():
    """Get set of all instruments with open positions"""
    try:
        ...
        for pos in resp.get("positions", []):
            ...
            if any(float(pos.get(side, {}).get("units", 0)) != 0 for side in ("long", "short")):
                open_set.add(instr)
        return open_set
    except Exception as e:
...
def get_candles(instrument: str, granularity: str = "D", count: int = 50, start=None, end=None) -> list:
    """Fetch complete candles only"""
    ...
    try:
        ...
        return [c for c in oanda_client.request(req).get("candles", []) if c.get("complete")]
    except Exception as e:
