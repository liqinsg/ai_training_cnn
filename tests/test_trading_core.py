...
from utils.trading_core import (
    OANDA_ENV,  # import from the utils instead of config
    forex_market_closed,
    get_open_position,
...
# tests/integration/test_oanda_live_practice.py


def test_real_trade_lifecycle(signal='BUY'):

    assert (
        OANDA_ENV.lower() == "practice"
    ), "Refusing to trade on LIVE account"
