...
if TIMEFRAME in ("H4", "1H", "15m"):
    MCConfig.set_timeframe(TIMEFRAME, {
        "YF_INTERVAL": config_bot.YF_INTERVAL,
        "YF_PERIOD_FULL": config_bot.YF_PERIOD_FULL,
        "YF_PERIOD_RESAMPLE": config_bot.YF_PERIOD_RESAMPLE,
        "MC_LOOKBACK": cfg("H4_LOOKBACK", 90),
        "MC_FORECAST": cfg("H4_FORECAST", 8),
        "PERIODS_YEAR": config_bot.PERIODS_YEAR_D * 6,
        "MC_REPORT_TITLE": config_bot.MC_REPORT_TITLE,
        "RESULTS_DIR": RESULTS_DIR,
    })
else:
    MCConfig.set_timeframe(TIMEFRAME, {
        "YF_INTERVAL": config_bot.YF_INTERVAL_D,
        "YF_PERIOD_FULL": config_bot.YF_PERIOD_FULL_D,
        "YF_PERIOD_RESAMPLE": config_bot.YF_PERIOD_RESAMPLE_D,
        "MC_LOOKBACK": cfg("DAILY_LOOKBACK", 90),
        "MC_FORECAST": cfg("DAILY_FORECAST", 5),
        "PERIODS_YEAR": config_bot.PERIODS_YEAR_D,
...
STRAT_CFG = StrategyConfig(
    mode=config.MODE,
    min_conviction_score= 40.0 if config.MODE == "LEVEL10" else config.MIN_CONVICTION_SCORE,
    base_min_edge=0.50 if config.MODE == "LEVEL10" else 0.51,
...
def main():
    ...
    for pair in DEFAULT_PAIRS:
        ...
        # ── Execute or display
        if True or not args.test_trade:
