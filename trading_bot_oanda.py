...
def close_pos():
    try:
        [api.request(trades.TradeClose(OANDA_ACCOUNT_ID_1, tradeID=x["id"])) for x in api.request(trades.TradesList(OANDA_ACCOUNT_ID_1, params={"state":"OPEN","instrument":PAIR})).get("trades",[])]
        return True
    except: return False
...
def send_order(act, p):
    ...
    try: api.request(orders.OrderCreate(OANDA_ACCOUNT_ID_1,data=payload));print(f"{act} OK");return True
