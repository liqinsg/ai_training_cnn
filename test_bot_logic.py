...
def close_all():
    ...
    try:
        ...
        [api.request(trades.TradeClose(OANDA_ACCOUNT_ID,tradeID=t["id"])) for t in api.request(trades.TradesList(OANDA_ACCOUNT_ID,params={"state":"OPEN"})).get("trades",[])]
        [api.request(orders.OrderCancel(OANDA_ACCOUNT_ID,orderID=o["id"])) for o in api.request(orders.OrderList(OANDA_ACCOUNT_ID,params={"state":"PENDING"})).get("orders",[])]
    except:pass
