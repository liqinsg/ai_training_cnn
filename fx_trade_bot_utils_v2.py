...
def open_oanda_order_simple(
    ...
    try:
        ...
        if "orderFillTransaction" in resp:
            trade_id = str(
                resp["orderFillTransaction"].get("tradeOpened", {}).get("tradeID", "")
            )
...
def get_account_equity(api, oanda_account_id: str) -> float:
    try:
        ...
        return float(
            api.request(AccountDetails(accountID=oanda_account_id))["account"][
                "balance"
            ]
        )
    except Exception as e:
...
def should_close_by_strength(
    ...
    base, quote = clean[:3], (
        clean[3:]
        if len(clean) == 6
        else ((lambda p: (p[0], p[1]))(pair.replace("=X", "").split("_")))
    )
    base_score, quote_score = strength_scores.get(base, 0), strength_scores.get(
        quote, 0
    )
...
def build_mc_telegram(
    mc_results: list, title: str, tf: str, lookback: int, forecast: int, sims: int
) -> str:
...
def compute_sl_zone(api, oanda_instrument, direction, entry_price, pip_size, cfg_fn):
    ...
    sl_fix = (
        entry_price - FIXED_PIPS * pip
        if direction == "BUY"
        else entry_price + FIXED_PIPS * pip
    )
...
PROFILE_BALANCED = PositionManagerProfile(
    ...
    trail_atr_mult=2.0,
    max_hold_bars=12,
    jpy_trail_factor=1.25,
)
PROFILE_TREND_FOLLOWING = PositionManagerProfile(
    be_trigger_atr_mult=3.0,
    trail_trigger_atr_mult=4.5,
    trail_atr_mult=3.0,
    max_hold_bars=24,
    jpy_trail_factor=1.20,
)
...
class DynamicPositionManager:
    ...
    def update_all(self, pair_data: dict, close_position_fn=None):
        ...
        for pair, info in pair_data.items():
            ...
            active_trail_mult = (
                self.trail_mult * self.jpy_trail_factor if is_jpy else self.trail_mult
            )
            ...
            for trade in trades:
                ...
                current_sl = (
                    float((trade.get("stopLossOrder") or {}).get("price"))
                    if (trade.get("stopLossOrder") or {}).get("price")
                    else None
                )
                current_tp = (
                    float((trade.get("takeProfitOrder") or {}).get("price"))
                    if (trade.get("takeProfitOrder") or {}).get("price")
                    else None
                )
                ...
                if bars_held >= self.max_hold:
                    logger.info(
                        f"⏰ TIME EXIT {pair} #{tid} — held {bars_held:.1f} bars"
                    )
                ...
                if profit_pips >= be_pips:
                    be_sl = round(
                        entry - pip if side == "long" else entry + pip, decimals
                    )
                    if (
                        current_sl is None
                ...
                if self.dynamic_tp and not in_min_hold:
                    if side == "long":
                        tp_candidate = round(
                            current_price + self.tp_atr_mult * atr_val, decimals
                        )
                        if (
                            current_tp is None
                            or tp_candidate > current_tp + self.tp_thresh_pips * pip
                        ):
                            self._update_trade_tp(
                                tid, instrument, tp_candidate, decimals
                            )
                    else:
                        tp_candidate = round(
                            current_price - self.tp_atr_mult * atr_val, decimals
