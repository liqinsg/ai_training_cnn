...
def close_pos():
    try:
        [api.request(trades.TradeClose(OANDA_ACCOUNT_ID_1,tradeID=t["id"])) for t in api.request(trades.TradesList(OANDA_ACCOUNT_ID_1,params={"state":"OPEN","instrument":PAIR})).get("trades",[])]
        return True
    except:return False
...
def do_order(act,p):
    ...
    try:api.request(orders.OrderCreate(OANDA_ACCOUNT_ID_1,data=pay));print("OK");return True
