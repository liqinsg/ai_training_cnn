# fx_trade_bot_v6.8.2_profile3.py — v6.8.2 | Profile3 / Account 003
...
# 🔑 Account: 003 — from config_bot_profile3
...
# ─── IMPORT CHAIN: profile3 → config_bot → config → defaults ───
import config_bot
import config
import config_bot_profile3  # ✅ YOUR Profile3 settings
...
# ──────────────────────────────────────────────────────
# ✅ Get Account ID from config_bot_profile3
# ──────────────────────────────────────────────────────
from config_bot_profile3 import OANDA_ACCOUNT_ID
...
# -----------------------------------------------------------------------------
# Unified Config Lookup: config_bot_profile3 → config_bot → config → default
# -----------------------------------------------------------------------------
...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler(BASE_DIR / "bot_profile3.log"), logging.StreamHandler(sys.stdout)],
)
logger = logging.getLogger(__name__)


def cfg_bot(name, default):
    """Lookup: config_bot_profile3 → config_bot → config → default"""
    return getattr(config_bot_profile3, name,
...
# ══════════════════════════════════════════════════════════════
# ✅ ALIGNED WITH PROFILE2 — WEIGHTS & THRESHOLDS
# ══════════════════════════════════════════════════════════════
...
# P2: ✅ ALIGNED WEIGHTS — S=0.40 R=0.15 A=0.15 X=0.20 M=0.10
...
logger.info(f"⚖️  PROFILE3 WEIGHTS: S={W_S:.2f} R={W_R:.2f} A={W_A:.2f} X={W_X:.2f} M={W_M:.2f} | SUM=1.00")


# 🎯 Thresholds — ALIGNED WITH PROFILE2
MIN_STRENGTH_GAP = cfg_bot("MIN_SCORE_GAP", 0.25)
DEBUG_DETAIL = cfg_bot("DEBUG_DETAIL", True)


# ─── 🎯 AUTO-RANKING — ALIGNED WITH PROFILE2 ───
...
parser = argparse.ArgumentParser(description="FX Trading Bot v6.8.2 Profile3 | Account 003 | RSI-FIXED | S=0.40 X=0.20")
...
COOLDOWN_FILE = BASE_DIR / "cooldown_profile3.json"  # ✅ Separate cooldown!
RESULTS_DIR = BASE_DIR / "daily_results_profile3"   # ✅ Separate MC results!
...
if TIMEFRAME in ("H4", "1H", "15m"):
    MCConfig.set_timeframe(TIMEFRAME, {
        ...
        "MC_REPORT_TITLE": "[PROFILE3] FX H4 MONTE CARLO",
    ...
    MCConfig.set_timeframe(TIMEFRAME, {
        ...
        "MC_REPORT_TITLE": "[PROFILE3] FX DAILY MONTE CARLO",
        "RESULTS_DIR": RESULTS_DIR,
    })
...
# -----------------------------------------------------------------------------
# ✅ v6.8.2 — RSI FIXED: Direction-Aware Scoring — EXACTLY FROM PROFILE2
# -----------------------------------------------------------------------------
def calc_weighted_score(pair: str, gap: float, rsi_val: float, adx_val: float,
    ...
    # ══════════════════════════════════════════════════════════════
    # SCORE CALCULATION — ALIGNED WEIGHTS S=0.40 R=0.15 A=0.15 X=0.20 M=0.10
    # ══════════════════════════════════════════════════════════════
    ...
    # 2. ✅ RSI — DIRECTION-AWARE FIXED LOGIC (matches Profile2)
...
# -----------------------------------------------------------------------------
# Main Trading Flow — v6.8.2 PROFILE3 — ALIGNED WITH PROFILE2
# -----------------------------------------------------------------------------
def main():
    global model_wrapper, strat_engine
    logger.info(f"\n🤖 RUN v6.8.2 PROFILE3 — Account 003 | RSI-FIXED | S=0.40 X=0.20 | {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')} | "
    ...
    if forex_market_closed():
        ...
        send_telegram_message("⏸️ FX BOT PROFILE3: Market closed")
        return
    ...
    if not pair_data:
        ...
        send_telegram_message("❌ FX BOT PROFILE3: No usable data")
        return
    ...
    # ─── STEP 6: Scan Open Positions — ALIGNED WITH PROFILE2 ───
    ...
    for pair in selected_pairs:
        ...
        try:
            prices = get_live_prices(oanda)  # ✅ Using shared function from Profile2
        ...
        # Gap Filter — ALIGNED WITH PROFILE2 MIN_GAP = 0.25
    ...
    if trade_lines:
        summary = f"🤖 v6.8.2 PROFILE3 RUN COMPLETE — Account 003 — {signal_count} SIGNAL(S)\n\n" + "\n".join(trade_lines)
        send_telegram_message(summary)
    else:
        logger.info("📋 No signals passed thresholds")
        if args.test_trade:
            send_telegram_message("🤖 v6.8.2 PROFILE3 TEST — No signals passed threshold")


    logger.info("✅ v6.8.2 PROFILE3 Run Complete — Account 003 — RSI-FIXED Applied | S=0.40 X=0.20")
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.exception("Fatal error in PROFILE3 main loop")
        send_telegram_message(f"❌ FX BOT PROFILE3 FATAL ERROR: {e}")
        raise